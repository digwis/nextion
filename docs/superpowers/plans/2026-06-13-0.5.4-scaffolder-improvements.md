...
**New files**
- `packages/create-notionx-app/src/templates/components/site/theme-bootstrap.tsx.tmpl` — sets CSS variables for theme colors/fonts picked from Notion
- `packages/create-notionx-app/src/__tests__/site-settings-schema.test.ts` — snapshot + helper tests for the new schema

**Modified files (scaffolder source)**
- `packages/create-notionx-app/src/provision/notion.ts` — `buildSiteSettingsProperties` (5→17 props), `buildSiteSettingsSeedPage` (seed all 17), `ensureSiteSettingsDatabase` (reuse via stable key + title fallback), add `SiteSettingsProvisionResult` interface with `reused: boolean`
- `packages/create-notionx-app/src/provision/notion.test.ts` — new tests for reuse, stable-key marker, schema count, seed coverage
- `packages/create-notionx-app/src/provision/index.ts` — pass `reused` through; pick ♻️/🆕 emoji in summary line
- `packages/create-notionx-app/src/prompt.ts` — remove `UI_PRESETS` table, remove `uiPreset` interactive prompt, remove `uiPreset` field on `Answers`, drop `UiPreset` type, drop `isUiPreset`
- `packages/create-notionx-app/src/answers.ts` — drop `--ui` / `--ui-preset` / `CREATE_NEXTION_UI` / `CREATE_NEXTION_UI_PRESET` handling; drop `uiPreset` from `applyDefaults` result
- `packages/create-notionx-app/src/presets.ts` — delete `MINIMAL_PRESET` and `APP_PRESET`; keep `SITE_PRESET` and the helper functions
- `packages/create-notionx-app/src/presets.test.ts` — remove `minimal` and `app` cases; pin `site` as the only preset
- `packages/create-notionx-app/src/ui-presets.ts` — keep only `site` (drop the array, drop `normalizeUiPreset`'s `minimal`/`app` branches)
- `packages/create-notionx-app/src/render.ts` — hard-code `uiPreset = "site"`; drop the `answers.uiPreset` plumbing
- `packages/create-notionx-app/src/render.test.ts` — remove "minimal" rendering test; drop `uiPreset` cases
- `packages/create-notionx-app/src/metadata.ts` — drop `uiPreset` from scaffold metadata shape
- `packages/create-notionx-app/src/__tests__/metadata.test.ts` — drop `uiPreset` assertions
- `packages/create-notionx-app/src/update/update-answers.ts` — drop `uiPreset` propagation
- `packages/create-notionx-app/src/provision/repair.ts` — drop `uiPreset` from `UpdateAnswers`
- `packages/create-notionx-app/src/update/update.test.ts` + `packages/create-notionx-app/src/provision/repair.test.ts` — drop `uiPreset` literals

**Modified files (templates)**
- `packages/create-notionx-app/src/templates/lib/site/config.ts.tmpl` — add `seo`, `footer`, `theme` to `fallbackSiteConfig`
- `packages/create-notionx-app/src/templates/lib/site/settings.ts.tmpl` — add `readJson` helper; read 12 new fields; merge into `SiteConfig`
- `packages/create-notionx-app/src/templates/app/layout.tsx.tmpl` — use new SEO fields; emit `data-theme` / `data-font` on `<html>`
- `packages/create-notionx-app/src/templates/components/site/site-header.tsx.tmpl` — multi-level nav with dropdowns + optional CTA button
- `packages/create-notionx-app/src/templates/components/site/site-footer.tsx.tmpl` — multi-column footer driven by `footer.columns` / `footer.social` / `footer.tagline` / `footer.copyright`

**Unchanged (intentionally)**
- `packages/create-notionx-app/src/templates/lib/content/models.ts.tmpl` — `siteSettingsSource` stays as-is; new fields are read directly off raw Notion properties (same pattern the existing 5 fields use)
- `packages/create-notionx-app/src/templates/components/ui/*.tmpl` — file set stays the same; the `site` preset already matches what's in the directory
...
**Files:**
- Modify: `packages/create-notionx-app/src/provision/notion.ts:1616-1626`
...
Run: `cd packages/create-notionx-app && pnpm vitest run src/provision/notion.test.ts -t "site-settings reuse"`
...
**Files:**
- Modify: `packages/create-notionx-app/src/provision/notion.ts:1701-1780`
...
Run: `cd packages/create-notionx-app && pnpm vitest run src/provision/notion.test.ts`
Expected: all pass
...
**Files:**
- Modify: `packages/create-notionx-app/src/provision/index.ts:344-363, 820-849`
...
```bash
git add packages/create-notionx-app/src/provision/index.ts
...
**Files:**
- Modify: `packages/create-notionx-app/src/provision/notion.ts:1642-1651`
- Test: `packages/create-notionx-app/src/provision/notion.test.ts` (replace the existing "site-settings builders" block)
...
Run: `cd packages/create-notionx-app && pnpm vitest run src/provision/notion.test.ts -t "site-settings builders"`
...
Run: `cd packages/create-notionx-app && pnpm vitest run src/provision/notion.test.ts -t "seeds the singleton row with all 17 properties"`
...
Run: `cd packages/create-notionx-app && pnpm typecheck`
Expected: PASS (template files are not type-checked directly, but the consuming code will check at scaffold time)

- [ ] **Step 3: Commit**

```bash
git add packages/create-notionx-app/src/templates/lib/site/config.ts.tmpl
...
Run: `cd packages/create-notionx-app && pnpm test`
Expected: PASS (the new code is template-side; no unit tests reference it directly yet)

- [ ] **Step 6: Commit**

```bash
git add packages/create-notionx-app/src/templates/lib/site/settings.ts.tmpl
...
**Files:**
- Modify: `packages/create-notionx-app/src/templates/app/layout.tsx.tmpl`
...
```bash
git add packages/create-notionx-app/src/templates/app/layout.tsx.tmpl
...
**Files:**
- Create: `packages/create-notionx-app/src/templates/components/site/theme-bootstrap.tsx.tmpl`
...
```bash
git add packages/create-notionx-app/src/templates/components/site/theme-bootstrap.tsx.tmpl \
        ...
        packages/create-notionx-app/src/templates/app/layout.tsx.tmpl
...
Run: `cd packages/create-notionx-app && pnpm vitest run src/render.test.ts src/__tests__/metadata.test.ts`
...
```bash
git add packages/create-notionx-app/src/prompt.ts packages/create-notionx-app/src/answers.ts
...
**Files:**
- Modify: `packages/create-notionx-app/src/presets.ts`
- Modify: `packages/create-notionx-app/src/presets.test.ts`
- Modify: `packages/create-notionx-app/src/ui-presets.ts`
...
```bash
git add packages/create-notionx-app/src/presets.ts \
        packages/create-notionx-app/src/presets.test.ts \
...
- Modify: `packages/create-notionx-app/src/__tests__/metadata.test.ts`
- Modify: `packages/create-notionx-app/src/update/update-answers.ts`
...
**Files:**
- Modify: `packages/create-notionx-app/package.json` (version bump driven by `pnpm changeset version`)

- [ ] **Step 1: Run `pnpm changeset version`**

Run: `cd /Users/zhao/项目/nextion && pnpm changeset version`
Expected: changesets consumed, `packages/create-notionx-app/package.json` bumped to `0.5.4`, CHANGELOG updated

- [ ] **Step 2: Verify the version**

Run: `grep version packages/create-notionx-app/package.json`
Expected: `"version": "0.5.4"`

- [ ] **Step 3: Commit the version bump**

```bash
git add packages/create-notionx-app/package.json \
        packages/create-notionx-app/CHANGELOG.md \
        .changeset
