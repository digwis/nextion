 ...
 */
export interface FoundationMiddlewareOptions {
  ...
  /**
   * Override which paths are gated. The default is the foundation's
...
export interface FoundationMiddlewareRequestContext {
  viewer: FoundationViewer | null;
  sessionId: string | null;
}

const requestContext = new WeakMap<Request, FoundationMiddlewareRequestContext>();
...
export async function resolveFoundationViewer(
  request: Request,
  options: FoundationMiddlewareOptions
): Promise<FoundationMiddlewareRequestContext> {
 ...
 * the request did not pass through `foundationMiddleware`.
 */
export function getFoundationContext(
  request: Request
): FoundationMiddlewareRequestContext | undefined {
 ...
 *
 * Side effect: attaches a `FoundationMiddlewareRequestContext` to
 * the request via a `WeakMap` so downstream route handlers can
 * resolve the viewer without a global.
 */
export async function foundationMiddleware(
  request: Request,
  env: unknown,
  options: FoundationMiddlewareOptions
 ...
 */
export function createFoundationMiddleware(
  options: FoundationMiddlewareOptions
): (request: Request, env: unknown) => Promise<Response | null> {
  return (request, env) => foundationMiddleware(request, env, options);
}
