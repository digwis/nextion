...
import { foundationMiddleware, type FoundationMiddlewareOptions } from "../middleware";
import type {
  AdminNavItem,
  AuthConfig,
  ContentSource,
...
export interface FoundationWorkerOptions {
   ...
   * `foundationMiddleware`. When omitted the middleware still runs
   * the admin gate but always reports no viewer.
   */
  sessionLookup?: FoundationMiddlewareOptions["sessionLookup"];
  /**
 ...
 *   1. Runs `foundationMiddleware` (admin gate + viewer attachment).
...
export function createFoundationWorker(
  ...
  const middlewareOptions: FoundationMiddlewareOptions = {
  ...
  return {
    async fetch(request, env, ctx) {
      ...
      const gateResponse = await foundationMiddleware(
        request,
        env,
        middlewareOptions
      );
